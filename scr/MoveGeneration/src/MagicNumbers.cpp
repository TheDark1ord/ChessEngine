...
bitboard movgen::get_bishop_attacks(bitboard blocker, bpos square,
...
bitboard movgen::get_rook_attacks(bitboard blocker, bpos square,
                                  movgen::GeneratedMagics *gen_m) {
...
void movgen::generate_magics(movgen::GeneratedMagics *gen_m) {
  for (bpos pos = 0; pos < 64; pos++) {
    ...
    while (true) {
      ...
      if (count_1s((bishop_magic.mask * bishop_magic.magic) &
    ...
    while (true) {
      ...
      if (count_1s((rook_magic.mask * rook_magic.magic) &
                   0xFF00000000000000ull) < 6)
        continue;
